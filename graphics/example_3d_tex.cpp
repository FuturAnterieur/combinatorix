...
std::string get_shader_source(const std::string &file_path){
  std::ifstream file(file_path.data(), std::ifstream::in);
  std::string source;
...
int main() {
    ...
    pip_desc.layout.buffers[0].stride = 3 * sizeof(float);

    pip_desc.layout.buffers[1].stride = 3 * sizeof(float);
    ...
    pip_desc.layout.attrs[1].offset = 0; 
